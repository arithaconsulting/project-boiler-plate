...
export const decisionPostUpVote = (authToken, postId, status) => dispatch =>
  dispatch({
    type: DECISION_POST_UPVOTE,
    payload: {
      request: {
        url: `api/v1/post/${postId}/activity/UPVOTE`,
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${authToken}`,
...
export const commentUpVoteApi = (authToken, commentId, payload) => dispatch =>
  dispatch({
    type: COMMENT_UPVOTE,
    payload: {
      request: {
        url: `api/v1/post/comment/${commentId}/activity/UPVOTE`,
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${authToken}`,
